...
代码编辑器允许您使用C＃代码编写初始化逻辑。可以在编辑器中输入以下代码行，然后单击“确定”进行编译和关闭。<br>
注意：符号要使用英文。
...
#### 打开表单测试初始值
在谷歌浏览器地址栏输入：http://localhost:8080 →在首页页面中单击“事务管理”图标，→进入“事务数据表格”页面，→单击右上角“+添加”按钮，弹出“事务表单”页面，可以看到预先设置的初始值，如下图：
