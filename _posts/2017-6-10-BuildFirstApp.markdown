...
Then, enter class name and caption as Issues, and click "OK" button. Notice that an icon called "Issues" appears under the "Classes" node which represents the new class.

Step 4: Create “ID” Attribute

  After the class is created, you will create properties of the class according to the previous definition. The first Attribute to create is the ID attribute. Since ID is a unique identifier for an issue and is generated by Ebaas, it takes some extra care when creating the attribute. 
...
Fig. 4. Settings of “ID” attribute

Step 5: Create “Subject” attribute

Creating “Subject” attribute is similar to that of “ID” attribute, except that the settings are little different. The data type is “String” and “Is Required” is True (See Fig. 5). When an attribute is required, it gets validated in a form submit.

Fig. 5, Settings of “Subject” attribute
...
Fig. 6: Setting of “Description” attribute
...
The way to create a List constraint is to add a List constraint with a name “IssueSubmitter” to the “Constraints” node in the tree (See Fig. 7).

Fig.7: Add a List constraint

The settings of the List constraint is shown in Fig. 8, where the “Newtera.Common.MetaData.UsersListHandler,Newtera.Common” is the definition for a predefined list handler that generates registered users.

Fig. 8: Settings of the List constraint for “SubmitBy” attribute
...
Step 15: Set “ID” attribute as the primary key
...
Step 1: Open the SiteMapStudio tool (Start->Ebaas->SiteMapStudio).
First, make sure the Ebaas server is running. The Fig. 17 shows the steps of opening the SiteMapStudio as the admin user. The username for the admin user is "admin" and the default password is "admin" too. 
...
Click on the “Issue Tracking” item you just created, next step is to define the settings of the item (Fig. 19). The table 2 below gives you the settings should enter:
...
Now you can test the application you just built.
...
Once you have logged in, you should be able to see the “Issue Tracking” menu item at the bottom (Fig. 20), click the menu item to get to the “datagrid” page for issues.

Fig. 20: Main menu

Step 1: Add an issue

The grid is empty at the first time. You can add an instance by clicking the green "+Add" button on top right of the grid, and a form comes up with the fields for the attributes of the “Issues” class (See Fig.21). This form is generated by the server for each class by default. You can create a customized form using a Form Designer which will be covered in other post.
...
The datagrid and form are two of very powerful UI components. We will explain their advanced features in the advanced topics.
