...
使用文本编辑器打开C:\Program Files\Ebaas\Config\Elasticsearch_config.xml文件，将“ServerUrl”参数设置为“http://localhost:9200”，如下所示。
保存配置文件后，重启Ebaas服务器即可。

接下来，使用所推荐的浏览器登陆到“http://localhost:8080”。登陆为任何用户，会看到页面的上端出现了全文检索的查询框，见下图。表示基于ES的全文检索已经开启。但是搜索功能还不能使用，因为我们还没有配置和创建任何索引。
